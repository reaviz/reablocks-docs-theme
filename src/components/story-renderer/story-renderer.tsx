...
const extractFunctionFromOriginalFile = async (
  ...
  try {
    const apiUrl = `/api/story-source?storyPath=${encodeURIComponent(storyPath)}&functionName=${encodeURIComponent(functionName)}`;
    ...
    return `// Error reading file for ${functionName}: ${error instanceof Error ? error.message : String(error)}`;
  }
};
